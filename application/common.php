...
function tx($id){
	...
	return $list['tx'];
		...
		return '/static/img/uimg/u__chat-img10.jpg';
	}
}
...
function grouptx($id){
	...
	$list=Db::name('group')->where('id='.$id)->find();
	if($list['img']){
	return $list['img'];
	}else{
		return '/static/img/uimg/u__chat-img10.jpg';
...
function userid($id){
	...
	//$User=M('user');
	$data['name']=$id;
	$list=Db::name('user')->where($data)->find();
	return $list['id'];	
}
...
//验证是否为管理员登陆
function adminyz(){
	if(cookie('key') and cookie('admin')) {
...
{
	$list=Db::name('groupuser')->where('groupid',$id)->count();
	return $list;
}
...
{
	$data['groupid']=$id;
	$info = model('base')->listdbnum('groupuser',$data,9,'id desc');
	return $info;
}
