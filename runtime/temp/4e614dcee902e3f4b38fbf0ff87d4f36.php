<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:76:"/www/wwwroot/ql.bjoyhg.com/public/../application/admin/view/index/index.html";i:1578149360;s:63:"/www/wwwroot/ql.bjoyhg.com/application/admin/view/leftlist.html";i:1578149360;s:64:"/www/wwwroot/ql.bjoyhg.com/application/admin/view/rightlist.html";i:1561474068;}*/ ?>
<!DOCTYPE html>
<html>
  <head>
    <title>后台管理</title>
      ...
      <table class="table table-lightborder">
		...
		<?php if(is_array($grouplist) || $grouplist instanceof \think\Collection || $grouplist instanceof \think\Paginator): $i = 0; $__LIST__ = $grouplist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
          <tr class="del<?php echo $vo['id']; ?>">
            ...
            <td>
             <?php echo groupuser($vo['id']); ?>
            </td>
            <td>
              <?php echo useraccount($vo['userid']); ?>
            </td>
            <td class="text-center">
			<style type="text/css">
				input.cp__checkbox-switch {
    margin: 8px;
    width: 20px;
    height: 20px;
}
			...
			<div class="cnt">
			<a class="btn btn-sm btn-success" href="javascript:;">是否禁言</a><input class="cp__checkbox-switch" type="checkbox" onclick="groupzt(<?php echo $vo['id']; ?>)" <?php if($vo['zt'] == '1'): ?>checked<?php endif; ?>>
			<a class="btn btn-sm btn-secondary" href="javascript:;">是否显示</a><input class="cp__checkbox-switch" type="checkbox" onclick="groupxs(<?php echo $vo['id']; ?>)" <?php if($vo['rank'] == '1'): ?>checked<?php endif; ?>>
			</div>
			
			
			<!--
			<a class="btn btn-sm btn-warning" href="#">已审核</a>
			...
			<a class="btn btn-sm btn-info" href="javascript:;" onclick="groupqk(<?php echo $vo['id']; ?>)">清空</a>
...
<div class="element-wrapper">
  ...
  <div class="element-box">
    ...
    <script>
	    ...
	    if(confirm("是否确认删除")){			
		$.post("/index.php/operate/index/groupdel",
			{
		 id:id,//用户ID
		 },
	 function(data,status){
        ...
        function groupqk(id){
	    if(confirm("是否确认清空该聊天室所有记录")){			
		$.post("/index.php/operate/index/groupqk",
			{
		 id:id,//用户ID
		 },
	 function(data,status){
	   ...
	   function groupjy(id){
	    if(confirm("是否确认禁言")){			
		$.post("/index.php/operate/index/groupjy",
			{
		 id:id,//用户ID
		 },
	 function(data,status){
     ...
     //关闭聊天是发言
function groupzt(id){  
	 $.post("/index.php/operate/index/groupzt",
    {
      id:id,
	},
	   function(data,status){
		   if(data==2){
		    Toast('没有权限',2000);
			   ...
			   Toast('更改聊天室状态,不允许发言',3000);
		   }else{
			   Toast('更改聊天室状态,允许发言',3000);
		   }
	   });
  ...
  return;
     ...
     //是否显示聊天室
function groupxs(id){  
	 $.post("/index.php/operate/index/groupxs",
    {
      id:id,
	},
	   function(data,status){
		   if(data==2){
		    Toast('没有权限',2000);
			   ...
			   Toast('更改聊天室状态,不显示',3000);
		   }else{
			   Toast('更改聊天室状态,显示',3000);
		   }
	   });
